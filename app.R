...
ui <- dashboardPage(skin = "red",
                    ...
                    dashboardSidebar(
                      sidebarMenu(
                        ...
                        menuItem("CTR Percentage", tabName = "visualisasi")
                      ),
                      tags$style(
                        HTML("
        .main-sidebar {
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(data = data, data1 = data1, data2 = data2, selected_rows = NULL, editing_row = NULL)
  ...
  observeEvent(input$edit_data_btn, {
    if (!is.null(rv$selected_rows) && length(rv$selected_rows) == 1) {
      ...
      updateTextInput(session, "all_input", value = as.character(rv$data2[rv$editing_row]))
    }
  })
  ...
  observeEvent(input$submit_btn, {
    if (!is.null(rv$editing_row)) {
      ...
      rv$data2[rv$editing_row] <- as.numeric(input$all_input)
      rv$editing_row <- NULL
    }
  })
  ...
  output$output_anova <- renderPrint({
    ...
    #Display Anova test results for data1
    anova_result <- aov(CTR ~ Ad_Placement, data = rv$data1)
  ...
  output$box_all <- renderText({
    if (is.null(rv$data)) return(NULL)
    result_anova <- aov(CTR ~ Ad_Placement, data = rv$data1)
    p_value <- format(summary(result_anova)[[1]]$'Pr(>F)'[1], digits = 3)
    if (as.numeric(p_value) > 0.05) {
